...
# Node 0: API server + global DP ranks 0-15.
local_ip="${VLLM_NODE_LOCAL_IP:-10.44.53.212}"
...
if [ -z "$nic_name" ]; then
  ...
  echo "Set it explicitly, for example: VLLM_NIC_NAME=eth0 bash run_dp32/master.sh" >&2
  exit 1
fi
...
# Profile and log only node 0's global DP0 / TP0 worker.
...
echo "Starting node0 master on $local_ip ($nic_name)"
echo "Model: $model_path"
echo "Profiling global DP rank $VLLM_PROFILE_TARGET_DP_RANK, TP rank $VLLM_PROFILE_TARGET_TP_RANK"
echo "Profile output: $VLLM_PROFILE_DIR"
